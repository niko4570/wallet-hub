...
#### 2.2.6 Balance History 模块优化

**当前问题**：

- 只支持 24 小时时间范围，缺少 7 天和 30 天等更长时间单位的支持
- 缺少数据聚合或插值逻辑，数据点可能稀疏
- 只有空状态处理，没有加载状态
- 没有时间范围选择功能
- 没有数据点交互或详细信息展示
- 每次组件渲染都重新计算图表路径，没有缓存机制
- 只显示单个钱包的余额历史，不支持多钱包数据合并

**优化建议**：

1. **时间单位扩展**：支持 24h、7d、30d 等不同时间单位的数据显示
2. **智能数据处理**：根据所选时间单位自动调整数据聚合和插值逻辑
3. **用户体验增强**：添加加载状态、时间范围选择、数据点交互等功能
4. **性能优化**：使用 React.memo 和 useMemo 优化渲染性能，实现图表路径缓存
5. **多钱包支持**：实现多钱包余额历史数据的合并显示
6. **视觉效果提升**：增强图表的视觉效果，添加渐变填充、动态效果等

**优先级**：

1. **高优先级**：时间单位扩展，智能数据处理，加载状态和错误处理
2. **中优先级**：用户体验增强，性能优化，多钱包支持
3. **低优先级**：视觉效果提升，高级缓存机制
...
6. **Balance History 时间单位扩展**：实现 Balance History 模块对 24h、7d、30d 时间单位的支持
7. **加载状态和错误处理**：为 Balance History 模块添加加载状态和错误处理
...
7. **智能数据处理**：实现 Balance History 模块的智能数据聚合和插值逻辑
8. **用户体验增强**：为 Balance History 模块添加时间范围选择和数据点交互功能
...
7. **多钱包数据合并**：实现 Balance History 模块的多钱包数据合并功能
8. **高级缓存机制**：为 Balance History 模块实现高级缓存机制
9. **性能优化**：优化 Balance History 模块的渲染性能
10. **视觉效果提升**：增强 Balance History 模块的图表视觉效果，添加渐变填充和动态效果
