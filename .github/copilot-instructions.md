...
- **Mobile client**  
  ...
  - All wallet state (linked accounts, balance history, totals) lives in `store/walletStore.ts` (Zustand + AsyncStorage); update balances via the provided actions so charts and derived values stay consistent.  
  ...
  - UI composition: `screens/WalletScreen.tsx` renders the connect-first portfolio dashboards, while `screens/ActivityScreen.tsx` renders the merged transaction feed with on-screen detail modals.
- **Backend service**  
...
3. **Route Solana + Jupiter traffic through services** – `watchlistDataService` (account snapshots + activity), `jupiterPortfolioService`, `jupiterTransactionsService`, `priceService`, and `rpcService` embed caching, retries, and API key headers. Screen/components should dispatch store actions or call these services, never hit remote endpoints directly.
4. **State mutations go through `walletStore` actions** – whether updating balances or activity streams, prefer helpers such as `updateDetailedBalance`, `updateHistoricalBalance`, and `setLinkedWallets` to keep derived totals, charts, and persisted data coherent.
