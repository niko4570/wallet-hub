import { validateSolanaAddress, validateSolAmount, validateEmail, validatePassword, validateRequired, validateUrl, validatePositiveNumber } from '../validation';
...
describe('Validation utilities', () => {
  describe('validateSolanaAddress', () => {
    ...
    it('should return invalid for address that is too long', () => {
      const result = validateSolanaAddress('12345678901234567890123456789012345678901234'); // 45 characters
      expect(result.valid).toBe(false);
      expect(result.error).toContain('Wallet address must be between');
    });

    it('should return invalid for address with invalid characters', () => {
      const result = validateSolanaAddress('123456789012345678901234567890123456789012340'); // Contains '0' which is invalid
      expect(result.valid).toBe(false);
