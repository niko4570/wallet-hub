    ...
    <View style={styles.container}>
      ...
      >
        <View style={styles.iconContainer}>
          <IconLoader 
            walletId={isDetectedWallet ? wallet.id : wallet.walletAppId || 'unknown'} 
            size={40} 
          />
        </View>
        ...
        <View style={styles.infoContainer}>
          <Text style={styles.name}>
            {isDetectedWallet ? wallet.name : wallet.label || wallet.walletName || 'Unknown Wallet'}
          </Text>
          {isDetectedWallet && wallet.subtitle && (
            <Text style={styles.subtitle}>{wallet.subtitle}</Text>
          )}
          {isLinkedWallet && (
            <Text style={styles.subtitle}>{wallet.address.slice(0, 8)}...{wallet.address.slice(-8)}</Text>
          )}
        </View>
